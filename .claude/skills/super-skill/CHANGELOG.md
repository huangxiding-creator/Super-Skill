...
Fifth run of the external-wisdom self-learning pipeline. Source: 混沌学园《100% 可以落地的AI产品"方法论"，如何用AI做出新产品》(任鑫 · 34'23" 全文文稿, `E:\AI-Station\data\hundun\AI课程\100_*.md` — 授权目录就地精读, 语料本体永不入库). 对账结论: 本课属任鑫 14 门主讲课系列, 主干 (新能力×新场景×新人群盘点/每项拆两半、静态内容→定制化服务、99% 搭建难度下降、洞察=信息+推理) 已存于 [renxin-ai-product-methodology.md](references/renxin-ai-product-methodology.md) — skip 参见已有条目; 以下 7 项经 grep 全库对账确认为**真空增量**, 入新小节「新能力→产品转化」:

- **纸笔残留透镜** (思维模型): 数字产品的"理所当然"多是旧媒介残影——文档分页为打印而生、三段笔记挤一个文档为省纸而生、专业软件复杂界面为机器语法而生; 审计自家产品每个默认结构"这是用户需求还是纸笔遗产", 残影处即原生创新空间.
- **拿着锤子找钉子** (原则): 早期刻意把新能力生搬硬套进产品, 哪怕结果不佳——目的是打开思路、摆脱老框架内微调; 仅听案例≠掌握, 强行套用一次才算.
- **新能力十问扫描** (方法论/清单): ①产品按人类语言与逻辑运作 ②更多元的信息解读 ③生成不同形态内容 (图/音/视频/虚拟人) ④静态内容动态化为定制服务 ⑤感知环境与状态更准理解意图 ⑥感知后选最有效回应形态 ⑦"大学实习生"级普遍常识助手 ⑧强化专业常识打垂直 ⑨用户的任务真正需要什么洞察 ⑩信息+推理直接产出该洞察.
- **视觉少样本泛化** (能力事实, 吴恩达实验室实证): 5 个字泛化 14000 字字体、画两下即识别目标、一点即分类培养皿——标注瓶颈崩塌, 垂直视觉应用 (质检/监控/医学影像) 启动成本降一个量级, 选品勿再以"要标海量数据"为由否决.
- **数字人=交互非广播** (思维模型): 只播视频的数字人直播与录播无差别, 机会在千人千面——1000 个观众各自与 NPC 对话, 形态从广播变游戏式交互; 多模态表达差异化 = 每用户一个专属实例.
- **可汗式引导** (设计手法, 深化设计五原则第 5 条): AI 导师不直接给答案——推理判断用户当前困惑点与能力位置, 沿其兴趣向前一步提问引导 (Khanmigo 先例).
- **版权灰色两问** (原则+机会): 拿他人内容做 AI 服务 (风格仿写/知识 bot/讲书服务) 先过两问——①是否加了转化价值 ②是否从中获利; 先划底线, 再在底线之上找机会; "数千年知识资产×用户之间的鸿沟"= AI 服务化直通车 (Google 收录网站诉讼史: 今天的灰色地带是明天的行业默认).

### Wired
- `SKILL.md`: proposal-forge 任鑫括号加"新能力→产品十问"; Phase 4 加"新能力转化透镜 (纸笔残留/锤子找钉子/版权两问)"; reference 表 renxin 行扩写; **V4.1.14** 版本块 (V4.1.13 并入 V3.21.0–V4.1.13 区间行); hooks 流程框图压缩为单行序列 (body 502→487 行, 回到 <500 预算); footer → V4.1.14.
- `references/renxin-ai-product-methodology.md`: 来源行加第 100 课补蒸馏注记; 新小节「新能力→产品转化」(7 条); 金句 +2 ("拿着锤子找钉子" · "先划底线，再在底线之上找机会").
